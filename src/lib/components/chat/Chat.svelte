<script lang="ts">
	...
	const initNewChat = async () => {
		...
		const availableModels = $models
			.filter((m) => !(m?.info?.meta?.hidden ?? false))
			.filter((m) => m.owned_by === 'openai')  // 只保留 OpenAI 模型
			.map((m) => m.id);
		...
		if (selectedModels.length === 0 || (selectedModels.length === 1 && selectedModels[0] === '')) {
			// 优先查找 gpt-4.1-mini 或 gpt-4o-mini
			const targetModelId = $models.find((m) => {
				const modelIdLower = m.id.toLowerCase();
				return modelIdLower.includes('gpt-4.1-mini') || 
					   modelIdLower.includes('gpt-4o-mini') ||
					   modelIdLower === 'gpt-4o-mini';
			})?.id;
			
			if (targetModelId && availableModels.includes(targetModelId)) {
		...
		// 强制设置为固定模型
		const fixedModel = getFixedModel();
		if (fixedModel) {
			selectedModels = [fixedModel.id];
		} else if (selectedModels.length === 0 || selectedModels[0] === '') {
			// 如果找不到固定模型，使用第一个可用的 OpenAI 模型
	...
	const sendMessage = async (
		...
		for (const [_modelIdx, modelId] of selectedModelIds.entries()) {
			// 直接使用固定模型，不依赖 $models
			const model = modelId === 'gpt-4.1-mini' 
				? { id: 'gpt-4.1-mini', owned_by: 'openai', name: 'GPT-4.1 Mini' }
	...
	const sendMessageSocket = async (model, _messages, _history, responseMessageId, _chatId) => {
		...
		const res = await generateOpenAIChatCompletion(
			localStorage.token,
			{
				...
				model_item: model.id === 'gpt-4.1-mini' 
					? { id: 'gpt-4.1-mini', owned_by: 'openai', name: 'GPT-4.1 Mini' }
	...
	const stopResponse = async () => {
		...
		if (responseMessage) {
			...
			if (responseMessage.parentId) {
				const parentMessage = history.messages[responseMessage.parentId];
	...
	const getFixedModel = () => {
		...
		// 	const modelIdLower = m.id.toLowerCase();
		// 	return (modelIdLower.includes('gpt-4.1-mini') || 
		// 			modelIdLower === 'gpt-4.1-mini') &&
		// 		   m.owned_by === 'openai';
		// });
		
		// // 如果找不到，查找 gpt-4o-mini
		// if (!targetModel) {
		// 	targetModel = $models.find((m) => {
		// 		const modelIdLower = m.id.toLowerCase();
		// 		return (modelIdLower.includes('gpt-4o-mini') || 
		// 				modelIdLower === 'gpt-4o-mini') &&
		// 			   m.owned_by === 'openai';
		// 	});
		// }
		
		let targetModel = { id: "gpt-4.1-mini", owned_by: "openai", name: "GPT-4.1 Mini" };

		return targetModel;
	};

	const ensureFixedModel = () => {
		const fixedModel = getFixedModel();
		if (fixedModel) {
			// console.log('模型找到', fixedModel.id);
			selectedModels = [fixedModel.id];
		} 
	...
	const model = atSelectedModel ?? getFixedModel() ?? $models.find((m) => m.id === selectedModels[0]);
</script>
